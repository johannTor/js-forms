document.addEventListener('DOMContentLoaded', () => {
    document.getElementById('new-contact-form').addEventListener('submit', catchSubmit);
});

// Contact object that stores each contact's information
function Contact(name, email, phone, age) {
    this.name = name;
    this.email = email;
    this.phone = phone;
    this.age = age;
...
function add(newContact) {
    ...
    if(getContactIndex(contactList, newContact.email) != -1) {
        ...
        document.getElementById('new-contact-header').innerHTML = "Contact added!";
...
function listAll() {
    for(var i = 0; i < contactList.length; i++) {
        console.log(contactList[i].name + " " + contactList[i].email + " " + contactList[i].phone + " " + contactList[i].age);
    }
}
...
function displayInfoInHtml(displayedContact) {
    ...
    var ageField = document.createElement("li");
    ...
    ageField.innerHTML = "Age: " + displayedContact.age;
...
function showEditBox(currContact) {
    ...
    document.getElementById("edit-age").value = currContact.age;
...
function processEditInfo(oldInfo) {
    ...
    var editAge = document.getElementById("edit-age").value;
    ...
    var updatedContact = new Contact(editName, editEmail, editPhone, editAge);

    // If edit is successfull, update the DOM
...
// Prevent default submit events on the new contact form
function catchSubmit(ev) {
    ev.preventDefault();
    processNewContact();
}

// Validate the new contact info and pass it into the add function if it checks out
function processNewContact() {
    resetNewContactLabels();
    var nameWarning = document.getElementById("name-label");
    var emailWarning = document.getElementById("email-label");
    var usedEmailWarning = document.getElementById("used-email-label");
    var phoneWarning = document.getElementById("phone-label");
    var ageWarning = document.getElementById("age-label");
    var newName = document.getElementById("new-name").value;
    var newEmail = document.getElementById("new-email").value;
    var newPhone = document.getElementById("new-phone").value;
    var newAge = document.getElementById("new-age").value;
    // Regex for checking if names are letters only
    var nameRegex = /^[a-zA-Z]+$/;
    // Regex for requiring emails with one @ symbol, at least one character before and after the @, no whitespaces and only one period
    var emailRegEx = /^[^@\s]+@[^@\s\.]+\.[^@\.\s]+$/;
    // Regex for requiring phone numbers to be digits only
    var phoneRegex = /^[0-9]+$/;

    // If name is missing show a message
    if(newName == "") {
        nameWarning.style.display = "inline";
        nameWarning.innerHTML = 'Please provide a name!';
        return;
    } else if(!nameRegex.test(newName)) {
        nameWarning.style.display = "inline";
        nameWarning.innerHTML = 'Use letters only!';
        return;
    }
    ...
    if(newEmail == "") {
        ...
        usedEmailWarning.style.display = "none";
        emailWarning.innerHTML = "Please provide an email";
        return;
    } else if(!emailRegEx.test(newEmail)) {
        emailWarning.style.display = "inline";
        usedEmailWarning.style.display = "none";
        emailWarning.innerHTML = "Email is in the wrong format";
    ...
    if(newPhone == "") {
        ...
        phoneWarning.innerHTML = 'Please provide a phone number';
        return;
    } else if(!phoneRegex.test(newPhone)) {
        phoneWarning.style.display = "inline";
        phoneWarning.innerHTML = 'Phone number must be digits only';
        return;
    }

    // Check if age is missing or if it's below 16 or above 150 
    if(newAge == "") {
        ageWarning.style.display = "inline";
        ageWarning.innerHTML = "Please provide an age";
        return;
    } else if(parseInt(newAge) < 16 || parseInt(newAge) > 150) {
        ageWarning.style.display = "inline";
        ageWarning.innerHTML = "Age must be between 16 and 150";
        return;
    }

    var newContact = new Contact(newName, newEmail, newPhone, newAge);
    add(newContact);
    resetNewContactFields();
}

var newPerson = new Contact("John Johnson", "john@gmail.com", "1234455", "80")
var newPerson2 = new Contact("Sarah Samsonite", "sarh@sam.com", "9998833", "30");
var newPerson3 = new Contact("Bobby Smith", "bob@smith.com", "2229988", "42");
add(newPerson);
add(newPerson2);
add(newPerson3);
displayListInHtml(contactList);